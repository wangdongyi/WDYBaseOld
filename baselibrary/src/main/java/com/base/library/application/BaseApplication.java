...
import com.base.library.listen.ApplicationListen;
...
public class BaseApplication extends MultiDexApplication {
    public int count = 0;
    private static ArrayList<ApplicationListen> ApplicationListenArrayList = new ArrayList<>();
    ...
    public static void AddApplicationListen(ApplicationListen applicationListen) {
        ApplicationListenArrayList.add(applicationListen);
    }

    public static void RemoveApplicationListen(ApplicationListen applicationListen) {
        ApplicationListenArrayList.remove(applicationListen);
    }
    ...
    public void AppListen() {
        registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks() {

            @Override
            public void onActivityStopped(Activity activity) {
                WDYLog.v("BaseApplication", activity + "onActivityStopped");
                count--;
                if (count == 0) {
                    for (int i = 0; i < ApplicationListenArrayList.size(); i++) {
                        ApplicationListenArrayList.get(i).onActivityStopped();
                    }
                    WDYLog.v("BaseApplication", "切到后台  lifecycle");
                }
            }
            ...
            public void onActivityStarted(Activity activity) {
                WDYLog.v("BaseApplication", activity + "onActivityStarted");
                if (count == 0) {
                    for (int i = 0; i < ApplicationListenArrayList.size(); i++) {
                        ApplicationListenArrayList.get(i).onActivityStarted();
                    }
                    WDYLog.v("BaseApplication", "切到前台  lifecycle");
                }
                count++;
            }
